...
import { buildLocalePathname, formatMessage } from "@/lib/i18n";
      ...
      <main className="pt-[40px]">
        ...
        <section className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
          <div className="absolute inset-0">
            {images.map((src, index) => (
              ...
              >
                <Image
                  alt={formatMessage(messages.home.alts.hero, { index: index + 1 })}
          ...
          <div className="relative z-10 text-center px-6 max-w-[900px]">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-6">
              ...
              <Link href={getLocalizedHref("/services")} className="border border-white text-white px-12 py-5 font-label-caps uppercase tracking-widest text-sm hover:bg-white/10 transition-colors">
          ...
          <div className="max-w-[1380px] mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-end mb-[64px] gap-8">
              <div className="max-w-xl">
                <span className="font-label-caps text-primary tracking-widest uppercase block mb-4">{messages.home.mastery.label}</span>
                <h2 className="font-headline-xl text-on-surface text-4xl">{messages.home.mastery.title}</h2>
              </div>
              <p className="font-body-md text-on-surface-variant max-w-sm">
                {messages.home.mastery.description}
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 h-auto md:h-[800px]">
              <div className="md:col-span-8 group relative overflow-hidden bg-black">
                <Image
                  alt={messages.home.alts.signature}
                ...
                <div className="absolute bottom-0 left-0 p-10 bg-gradient-to-t from-black/80 to-transparent w-full">
                  <h3 className="text-white font-headline-md text-3xl mb-2">{messages.home.mastery.signature.title}</h3>
                  <p className="text-gray-300 font-body-md mb-4 max-w-md">{messages.home.mastery.signature.description}</p>
                  <span className="text-primary-container font-label-caps tracking-widest">{messages.home.mastery.signature.price}</span>
                </div>
              </div>
              <div className="md:col-span-4 flex flex-col gap-8">
                <div className="flex-1 group relative overflow-hidden bg-black">
                  <Image
                    alt={messages.home.alts.ritual}
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-8 bg-gradient-to-t from-black/80 to-transparent">
                    <h3 className="text-white font-headline-md text-2xl mb-1">{messages.home.mastery.ritual.title}</h3>
                    <span className="text-primary-container font-label-caps tracking-widest">{messages.home.mastery.ritual.price}</span>
                  </div>
                </div>
                <div className="flex-1 group relative overflow-hidden bg-black">
                  <Image
                    alt={messages.home.alts.beard}
                    className="w-full h-full object-cover opacity-70 group-hover:scale-105 transition-transform duration-700"
                  ...
                  />
                  <div className="absolute inset-0 flex flex-col justify-end p-8 bg-gradient-to-t from-black/80 to-transparent">
                    <h3 className="text-white font-headline-md text-2xl mb-1">{messages.home.mastery.beard.title}</h3>
                    <span className="text-primary-container font-label-caps tracking-widest">{messages.home.mastery.beard.price}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-[1380px] mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-[96px] items-center">
            <div className="relative order-2 md:order-1">
              <div className="aspect-[4/5] overflow-hidden border border-primary/30 relative">
                <Image
                  alt={messages.home.alts.heritage}
            ...
            <div className="order-1 md:order-2">
              <span className="font-label-caps text-primary-container tracking-widest uppercase block mb-6">{messages.home.legacy.label}</span>
              <h2 className="font-display-lg text-white text-4xl mb-8">{messages.home.legacy.title}</h2>
              <div className="space-y-6 opacity-80 font-body-lg text-lg">
                {messages.home.legacy.paragraphs.map((paragraph, index) => (
                  <p key={index}>{paragraph}</p>
                ))}
              </div>
              <div className="mt-12">
                <a className="inline-flex items-center gap-4 group" href="#">
                  <span className="font-label-caps uppercase tracking-[0.2em] group-hover:text-primary-container transition-colors">{messages.home.legacy.link}</span>
        ...
        <section className="py-20 bg-surface-container text-center">
          <div className="max-w-[800px] mx-auto px-6">
            ...
            <h2 className="font-display-lg text-on-surface text-4xl mb-8">{messages.home.cta.title}</h2>
